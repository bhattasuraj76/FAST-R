...
def export_test_deletion_parent_commits(commits, deleted_testcases_with_whole_file):
    ...
    df.to_csv(f"tdc-parents-sample/{project}_strict.csv")
...
for index, project in enumerate(projects_list):
    ...
    candidate_commits_list = []
    for commit in commits_list:
        deleted_tc_with_source_code = deleted_testcases_with_whole_file_df[
            (deleted_testcases_with_whole_file_df["Parent"] == commit)
            & (
                deleted_testcases_with_whole_file_df["Deleted With Source Code"]
                == "yes"
            )
        ]
        # Candidate commit should have 0 tests deleted with source code 
        if len(deleted_tc_with_source_code) == 0:
            candidate_commits_list.append(commit)
            
            
    export_test_deletion_parent_commits(
        candidate_commits_list, deleted_testcases_with_whole_file_df
    )
