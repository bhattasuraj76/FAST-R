...
def export_test_deletion_parent_commits(commits, deleted_testcases_with_whole_file):
    ...
    for commit in commits:
        ...
        # redundant tests
        redundant_df = deleted_tests_df[deleted_tests_df["Deleted With Source Code"] == "no"]
        redundant_tests = redundant_df["Removed Test Case"].values.tolist()
        data["Total Redundant Tests"].append(len(redundant_tests))
        data["Redundant Tests"].append(redundant_tests)
    ...
    df.to_csv(f"tdc-parents-sample/{project}.csv", index=False)
